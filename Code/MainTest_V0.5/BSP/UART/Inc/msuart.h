...
static inline bool MSUART_Is_Busy(MSUART *msu)  // Hardware Busy
{
  ...
  if (msu->tx_busy && (isrflags & USART_ISR_TC) &&
      (isrflags & USART_ISR_TXE_TXFNF)) {
    if (!msu->hw_rs485) {
      if (msu->sw_rs485_tx2rx) msu->sw_rs485_tx2rx(msu);
    }
  ...
  if (msu->rx_busy && (isrflags & USART_ISR_IDLE) &&
      !(isrflags & USART_ISR_BUSY)) {
...
MSUART_Start_Transfer_Result MSUART_Start_DMA_Transfer(
    MSUART *msu, bool tx, bool force_tx, void (*tx_cplt_callback)(MSUART *),
    const uint8_t *tx_buffer, const uint16_t tx_size, bool rx, bool force_rx,
    bool continue_rx, void (*rx_idle_callback)(MSUART *), uint8_t *rx_buffer,
...
/**** END OF FILE ******************* (C) WUJITECH 2019-2025
 * ******************** END OF FILE ****/
