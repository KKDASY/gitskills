...
static void MSUART_Commu_Init(MSUART *msu)
{
	...
	huart->Init.OverSampling = msu->baudrate > 5000000 ?  UART_OVERSAMPLING_8 : UART_OVERSAMPLING_16;//根据波特率选择过采样率
	huart->Init.OneBitSampling = UART_ONE_BIT_SAMPLE_ENABLE;//启动单bit采样
...
#elif defined(STM32G4)
    hdma_tx->Init.MemDataAlignment = DMA_MDATAALIGN_BYTE;//按照字节对齐
#endif	
...
void MSUART_Init(void)
{
    ...
    DISABLE_UARTX_TX_DMA_IRQ(3);//关闭TX DMA中断处理
    DISABLE_UARTX_RX_DMA_IRQ(3);//关闭RX DMA中断处理
#endif
#if UART4_ENABLE
